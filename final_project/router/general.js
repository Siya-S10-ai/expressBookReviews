...
public_users.get('/isbn/:isbn', async (req, res) => {
  try {
    ...
    const { data: allBooks } = await axios.get(`${baseUrl}/`);
...
public_users.get('/author/:author', async (req, res) => {
  try {
    ...
    const result = Object.keys(allBooks)
      ...
      .map((isbn) => ({ ...allBooks[isbn], isbn }));

    return res.status(200).json(result);
  } catch (error) {
    return res.status(500).json({ message: 'Error fetching book by author', error });
  }
});
...
public_users.get('/title/:title',async (req, res) => {
  try{
    ...
    return res.status(500).json({ message: 'Error fetching book by title', error });
  }
});
